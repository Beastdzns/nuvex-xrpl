...
const xrpUtils = {
  ...
  sendXRP: async function (fromWallet, toAddress, amount, client) {
    ...
    try {
      ...
      console.log(
        `🌊 Sending ${amountStr} XRP drops from ${fromWallet.address} to ${toAddress}...`
      );
      ...
      if (
        ...
        if (response.result.meta.TransactionResult === "tesSUCCESS") {
          console.log(
            `✅ Successfully sent ${amountStr} XRP drops. Transaction hash: ${response.result.hash}`
          );
      ...
      throw new Error(
        `Failed to send XRP from ${fromWallet.address} to ${toAddress}: ${error.message}`
      );
    } finally {
      if (shouldDisconnect && xrplClient) {
        await xrplClient.disconnect();
...
async function initChain(cnf) {
  ...
  console.log(
    `[${cnf.chainId}] Escrow factory contract deployed to`,
    escrowFactory
  );
...
class EVMToXRPLSwap {
  ...
  async initialize() {
    ...
    if (this.src.node) {
      ...
      console.log(
        `🌐 Anvil RPC running at: http://[${nodeAddress.address}]:${nodeAddress.port}`
      );
      console.log(
        `📊 You can interact with this fork using any Ethereum tools pointing to this RPC`
      );
      console.log(`🔍 To explore transactions visually, you can:`);
      console.log(
        `   • Use Foundry's cast: cast tx <hash> --rpc-url http://127.0.0.1:${nodeAddress.port}`
      );
      console.log(`   • Connect MetaMask to this RPC for manual exploration`);
      console.log(
        `   • Use any Ethereum explorer by configuring it to this RPC endpoint`
      );
    }
    console.log("");
    ...
    console.log(
      `📝 User XRPL: ${this.userXRPLWallet.address} (will receive XRP)`
    );
    console.log(`📝 Resolver EVM: ${await this.srcChainResolver.getAddress()}`);
    console.log(
      `📝 Resolver XRPL: ${this.resolverXRPLWallet.address} (provides XRP)\n`
    );

    // Check initial balances
    const userUSDCBalance = await this.srcChainUser.tokenBalance(
      config.chain.source.tokens.USDC.address
    );
    console.log(
      `💰 User initial USDC balance: ${ethers.formatUnits(
        userUSDCBalance,
        6
      )} USDC`
    );
    ...
    const userXRPBalance = Number(
      xrpl.dropsToXrp(userXRPResponse.result.account_data.Balance)
    );
  ...
  async executeSwap() {
    ...
    const order = Sdk.CrossChainOrder.new(
      new Sdk.Address(this.src.escrowFactory),
      {
        ...
        ), // XRP (represented as native token)
      },
      {
    ...
    console.log(
      `🔗 EVM Transaction: http://localhost:${
        this.src.node?.address()?.port || "unknown"
      }/tx/${orderFillHash}\n`
    );
    ...
    const srcEscrowEvent = await this.srcFactory.getSrcDeployEvent(
      srcDeployBlock
    );
    ...
    const xrpEscrow = await this.nuvexClient.createDestinationEscrow(
      createEscrowPayload
    );
    ...
    console.log(
      `🔗 Deposit TX: https://testnet.xrpl.org/transactions/${resolverDepositHash}\n`
    );
    ...
    console.log(
      `🔗 Withdrawal TX: https://testnet.xrpl.org/transactions/${xrplWithdrawal.txHash}\n`
    );
    ...
    console.log(
      `✅ Resolver successfully withdrew USDC: ${resolverWithdrawHash}`
    );
    console.log(
      `🔗 EVM Transaction: http://localhost:${
        this.src.node?.address()?.port || "unknown"
      }/tx/${resolverWithdrawHash}\n`
    );
    ...
    const finalUserUSDC = await this.srcChainUser.tokenBalance(
      config.chain.source.tokens.USDC.address
    );
    const finalResolverUSDC = await this.srcResolverContract.tokenBalance(
      config.chain.source.tokens.USDC.address
    );
